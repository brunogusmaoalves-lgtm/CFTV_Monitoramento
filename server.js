...
io.on('connection', (socket) => {
    ...
    const ocorrencias = await Ocorrencia.find({}, { foto: 0 }).sort({ createdAt: -1 }).lean();
    const ufv_status = await UfvStatus.find({}, { foto1: 0, foto2: 0 }).sort({ nome: 1 }).lean();
    const visitas = await Visita.find().sort({ createdAt: -1 }).limit(6000).lean();
